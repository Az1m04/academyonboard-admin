...
const ClientFollowsUp = ({
  ...
  const getFollowUps = () => {
    ...
    if (record?.lastFollowUpBy?.department?.id) {
      dispatch({
        type: 'staff/getDepartmentStaffList',
        payload: {
          pathParams: { depId: record?.lastFollowUpBy?.department?.id },
        },
      });
    }

    if (record?.lastFollowUpBy && Object.keys(record?.lastFollowUpBy)?.includes('department')) {
      setAssignee(true);
    }
    if (record?.lastFollowUpBy?.isInterested === true) {
      setIsInterested(true);
    }
    if (record?.lastFollowUpBy?.isInterested === false) {
      setIsInterested(false);
    }
    form.setFieldsValue({
      followUpBy: {
        id:
          record?.lastFollowUpBy?.roleTypeId &&
          list?.find((val) => val?.name === record?.lastFollowUpBy?.roleTypeId)?.value,
      },
      lastStatus: record?.lastFollowUpStatus ? record?.lastFollowUpStatus : undefined,
      branch: {
        id: record?.lastFollowUpBy?.branch?.id ? record?.lastFollowUpBy?.branch?.id : undefined,
      },
      selectDepartment: record?.lastFollowUpBy?.department?.id
        ? record?.lastFollowUpBy?.department?.id
        : undefined,
      selectAssign:
        record?.lastFollowUpBy?.lastAssignee?.length > 0
          ? record?.lastFollowUpBy?.lastAssignee?.map((item) => item?.id)
          : undefined,
      currentActionMode: record?.currentActionId,
      isInterested: record?.lastFollowUpBy?.isInterested,
      comment: record?.lastFollowUpBy?.comment,
      feedBackType: record?.lastFollowUpBy?.feedBackType,
    });
  };
  ...
  const onCommentFinish = (value) => {
    ...
    if (data?.roleTypeId === 'EMPLOYEE') {
      data.followUpBy = { id: actionKey?.id };
    }
    if (!assignee) {
